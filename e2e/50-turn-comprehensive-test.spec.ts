...
async function testBuyPlanets(page: Page, turn: number) {
  ...
  try {
    ...
    if (!await planetsLink.isVisible().catch(() => false)) {
      logUXIssue("Planets navigation link not found", "Add clear navigation to Planets page");
      return;
    }
    ...
    logError(`Turn ${turn}: Planet purchase failed`, String(error));
  }
}
