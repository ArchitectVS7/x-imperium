...
describe("Civil Status Service", () => {
  describe("getIncomeMultiplier", () => {
    ...
    // This allows recovery from setbacks while still rewarding success
    it("should return correct multiplier for ecstatic status", () => {
      expect(getIncomeMultiplier("ecstatic")).toBe(2.5);
    });

    it("should return correct multiplier for happy status", () => {
      expect(getIncomeMultiplier("happy")).toBe(2.0);
    });

    it("should return correct multiplier for content status", () => {
      expect(getIncomeMultiplier("content")).toBe(1.5);
    });
    ...
    it("should return correct multiplier for unhappy status", () => {
      // Rebalanced: 15% penalty
      expect(getIncomeMultiplier("unhappy")).toBe(0.85);
    });

    it("should return correct multiplier for angry status", () => {
      // Rebalanced: 30% penalty
      expect(getIncomeMultiplier("angry")).toBe(0.7);
    });

    it("should return correct multiplier for rioting status", () => {
      // Rebalanced: 40% penalty
      expect(getIncomeMultiplier("rioting")).toBe(0.6);
    });

    it("should return correct multiplier for revolting status", () => {
      // Rebalanced: 50% penalty (near collapse, but recoverable)
  ...
  describe("evaluateCivilStatus - Upgrades", () => {
    it("should upgrade from content to happy on food surplus", () => {
      ...
      expect(result.multiplier).toBe(2.0); // Rebalanced: happy = 2.0
      expect(result.changed).toBe(true);
    });
  ...
  describe("evaluateCivilStatus - No Change", () => {
    it("should not change status with no events", () => {
      ...
      expect(result.multiplier).toBe(1.5); // Rebalanced: content = 1.5
      expect(result.changed).toBe(false);
    });
  ...
  describe("evaluateCivilStatus - Full Spectrum", () => {
    ...
    it("should return correct multipliers for all statuses", () => {
      // Rebalanced: 5x differential (2.5x to 0.5x) instead of 16x (4.0x to 0.25x)
      // Allows recovery from setbacks while still rewarding success
      const expectedMultipliers = [2.5, 2.0, 1.5, 1.0, 0.85, 0.7, 0.6, 0.5];
